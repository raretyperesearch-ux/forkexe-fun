...
export function useAgents(sourceFilter: string = 'all') {
  ...
  const fetchAgents = useCallback(async (isInitial = false) => {
    try {
      ...
      } else if (sourceFilter === 'doppler') {
        query = query.eq('source', 'doppler');
      } else if (sourceFilter === 'agent') {
        query = query.in('source', ['agent', 'bankr', 'moltbook', 'clawnch', 'doppler']);
      }
      ...
      if (['all', 'clawnch', 'clanker', 'doppler'].includes(sourceFilter)) {
        query = query.gte('liquidity', MIN_LIQUIDITY);
      }
