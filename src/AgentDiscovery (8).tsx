...
function PriceChart({ tokenAddress, isDark }: { tokenAddress: string; isDark: boolean }) {
  ...
  const fetchChartData = useCallback(async () => {
    ...
    try {
      // First get the pool address from token
      ...
      // Calculate aggregate and timeframe params
      let aggregate = '15'; // 15 minute candles
      let limit = 96; // Default for 24h (96 * 15min = 24h)
      ...
      if (timeframe === '1h') {
        ...
        aggregate = '60'; // 1 hour candles
        limit = 168;
      }
      
      // Fetch OHLCV data
      const ohlcvRes = await fetch(
        `https://api.geckoterminal.com/api/v2/networks/base/pools/${poolAddress}/ohlcv/minute?aggregate=${aggregate}&limit=${limit}&currency=usd`
      );
