...
// Shared caches for GeckoTerminal API (to avoid rate limiting)
const poolCache: Record<string, { pool: string, timestamp: number }> = {};
const chartCache: Record<string, { data: any[], timestamp: number, priceChange: number }> = {};
const tradesCache: Record<string, { trades: any[], timestamp: number }> = {};
const CACHE_TTL = 60000; // 1 minute for chart data
const TRADES_CACHE_TTL = 30000; // 30 seconds for trades
...
function PriceChart({ tokenAddress, isDark }: { tokenAddress: string; isDark: boolean }) {
  ...
  const fetchChartData = useCallback(async () => {
    if (!tokenAddress) {
      console.log('PriceChart: No token address provided');
      return;
    }
    ...
    if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
      console.log('PriceChart: Using cached data');
    ...
    console.log('PriceChart: Fetching for token:', tokenAddress);
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      const cachedPool = poolCache[tokenAddress];
      if (cachedPool && Date.now() - cachedPool.timestamp < CACHE_TTL) {
        poolAddress = cachedPool.pool;
        console.log('PriceChart: Using cached pool:', poolAddress);
      } else {
        // Fetch pool address from token via our proxy
        const poolsRes = await fetch(
          `/api/chart?token=${tokenAddress}`
        );
        const poolsData = await poolsRes.json();
        
        console.log('PriceChart: Pools response:', poolsData);
        
        if (poolsData.error?.includes('429')) {
        ...
        poolAddress = poolsData.data[0].attributes.address;
        
        // Cache the pool address
        poolCache[tokenAddress] = { pool: poolAddress, timestamp: Date.now() };
      }
  ...
  }, [tokenAddress, timeframe, cacheKey]);
      ...
      <div style={{ height: '280px', width: '100%' }}>
        {loading ? (
          ...
          <div style={{ 
            ...
            <div>{error}</div>
            <div style={{ fontSize: '10px', opacity: 0.5, wordBreak: 'break-all', padding: '0 20px', textAlign: 'center' }}>
              {tokenAddress ? `Token: ${tokenAddress.slice(0, 10)}...${tokenAddress.slice(-6)}` : 'No token address'}
            </div>
          </div>
        ) : (
...
function TransactionsFeed({ tokenAddress, isDark }: { tokenAddress: string; isDark: boolean }) {
  ...
  useEffect(() => {
    const fetchTrades = async () => {
      ...
      // Check trades cache
      const cachedTrades = tradesCache[tokenAddress];
      if (cachedTrades && Date.now() - cachedTrades.timestamp < TRADES_CACHE_TTL) {
        setTrades(cachedTrades.trades);
        setLoading(false);
        return;
      }
      ...
      try {
        // Check pool cache first
        let pool = poolCache[tokenAddress]?.pool;
        
        if (!pool) {
          // Fetch pool address
          const poolsRes = await fetch(`/api/chart?token=${tokenAddress}`);
          const poolsData = await poolsRes.json();
          ...
          pool = poolsData.data[0].attributes.address;
          poolCache[tokenAddress] = { pool, timestamp: Date.now() };
        }
        
        // Fetch trades
        const tradesRes = await fetch(`/api/chart?pool=${pool}&trades=true`);
        const tradesData = await tradesRes.json();
        
        if (tradesData.data) {
          const formatted = tradesData.data.slice(0, 20).map((t: any) => ({
            type: t.attributes.kind === 'buy' ? 'buy' : 'sell',
            amount_usd: parseFloat(t.attributes.volume_in_usd) || 0,
            timestamp: t.attributes.block_timestamp,
            tx_hash: t.attributes.tx_hash,
            price_usd: parseFloat(t.attributes.price_to_in_usd) || 0,
          }));
          setTrades(formatted);
          tradesCache[tokenAddress] = { trades: formatted, timestamp: Date.now() };
        }
        setLoading(false);
      } catch (err) {
    ...
    <div style={{ 
      ...
      <div 
        ...
        {trades.map((trade, i) => (
          <div
            key={trade.tx_hash + i}
            onClick={() => window.open(`https://basescan.org/tx/${trade.tx_hash}`, '_blank')}
            style={{
              flexShrink: 0,
              padding: '6px 10px',
              borderRadius: '8px',
...
function ScreenerPage() {
  ...
  const [sourceFilter, setSourceFilter] = useState<'all' | 'verified' | 'trending' | 'top10' | 'bankr' | 'clanker' | 'agent' | 'clawnch' | 'moltlaunch' | 'creatorbid' | 'trenches' | 'doppler'>('trending');
    ...
    if (sourceFilter !== 'all') {
      switch (sourceFilter) {
        ...
        case 'moltlaunch':
          ...
          break;
        case 'creatorbid':
          if (agentSource !== 'creatorbid') return false;
          break;
        case 'trenches':
          if (agentSource !== 'trenches') return false;
          break;
              ...
              <>
                ...
                <div style={{ 
                  ...
                  {[
                    ...
                    { key: 'clawnch', label: 'Clawnch' }, { key: 'moltlaunch', label: 'Moltlaunch' }, { key: 'creatorbid', label: 'CreatorBid' }, { key: 'trenches', label: 'Trenches' }, { key: 'doppler', label: 'Doppler' },
                ...
                <div style={{ 
                  ...
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                    {[
                      ...
                      { key: 'creatorbid', label: 'CreatorBid' },
      ...
      {selectedAgent && (
        <>
          ...
          <div style={{
            ...
            {/* Custom Price Chart */}
            <PriceChart 
              tokenAddress={(selectedAgent as any).token_address || (selectedAgent as any).tokenAddress || ''} 
              isDark={isDark} 
            />
